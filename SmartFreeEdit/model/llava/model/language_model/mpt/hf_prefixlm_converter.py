...
def _expand_mask_opt(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    """
    扩展注意力掩码 [bsz, seq_len] 到 [bsz, 1, tgt_seq_len, src_seq_len]
    """
...
def _expand_mask_bloom(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None) -> torch.Tensor:
    """
    扩展注意力掩码从维度 [bsz, seq_len] 到 [bsz, 1, tgt_seq_len, src_seq_len]
    """
    batch_size, source_len = mask.size()
    tgt_len = tgt_len if tgt_len is not None else source_len

    # 扩展维度并复制到目标长度
    expanded_mask = mask[:, None, None, :].expand(batch_size, 1, tgt_len, source_len).to(dtype)

    # 在 BLOOM 中,0 表示被掩盖(masked),1 表示未被掩盖(unmasked)
    # 因此需要将掩码反转并填充为很大的负值
    inverted_mask = 1.0 - expanded_mask
...
def _make_causal_mask_bloom(
    ...
    bsz, tgt_len = input_ids_shape
    mask = torch.full((tgt_len, tgt_len), True, device=device)  # 默认全部掩盖
    mask_cond = torch.arange(mask.size(-1), device=device)
    ...
    if past_key_values_length > 0:
        # 为增量解码添加额外的列
        mask = torch.cat(
            [torch.zeros(tgt_len, past_key_values_length, dtype=torch.bool, device=device), mask],
            dim=-1
        )

    # 扩展维度至 [batch_size, 1, tgt_len, src_len]
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)
